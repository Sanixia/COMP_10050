    ...
    if (j==0){
        ...
        for (i=0; i<MAX_TWEETS; i++)
        {
            while(j>(twitter_system->num_tweets-10) || j>0)
            {
                ...
                while (j > (twitter_system->num_tweets - 10) || j > 0) {
                    // check if the current tweet is made by the current user or if isFollowing returns a 1
                    // If either requirement is met the tweet is then printed aswell as the timestamp and user
                        ...
                        || isFollowing(twitter_system , user, j))
                    {
                        ...
                        || isFollowing(twitter_system, user, j)) {
                            printf("\nUser: %s\nTweet %d: %s", twitter_system->news_feed[j].user,
                                   twitter_system->news_feed[j].id + 1,
